...
`boxr wait` exits zero whenever it can report one of those outcomes, including an expired `--timeout`, and only exits non-zero when it cannot run at all: an unknown session or a ledger it cannot read.
