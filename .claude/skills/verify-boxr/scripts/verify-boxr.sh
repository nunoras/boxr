...
harness="${BOXR_VERIFY_HARNESS:-claude}"
model="${BOXR_VERIFY_MODEL:-haiku}"
effort="${BOXR_VERIFY_EFFORT-low}"
budget="${BOXR_VERIFY_TIMEOUT:-300}"
prompt="${BOXR_VERIFY_PROMPT:-Reply with the single word ok and nothing else.}"
...
cleanup() {
  code=$?
  if [ -n "$boxr_pid" ] && kill -0 "$boxr_pid" 2>/dev/null; then
    kill -TERM "$boxr_pid" 2>/dev/null || true
    sleep 1
    kill -KILL "$boxr_pid" 2>/dev/null || true
  fi
  case "$throwaway" in
    */boxr-verify.*) rm -rf "$throwaway" ;;
...
[ "$logged_in" = yes ] || die "no login in $config_dir; run '$harness' once to log in, or set ANTHROPIC_API_KEY"
...
if [ "$timed_out" = yes ]; then
  kill -TERM -"$boxr_pid" 2>/dev/null || kill -TERM "$boxr_pid" 2>/dev/null || true
  sleep 2
  kill -KILL -"$boxr_pid" 2>/dev/null || true
fi
exit_code=0
...
step="evidence"
{
  ...
  printf 'prompt: %s\n' "$prompt"
  printf 'boxrHomeRemoved: %s\n' "$throwaway"
  printf 'workDirRemoved: %s\n' "$work"
  printf 'exitCode: %s\n' "$exit_code"
  printf 'sessionId: %s\n' "$session_id"
  printf 'harnessSessionId: %s\n' "$harness_session_id"
  printf 'harnessTranscriptSource: %s\n' "${source_transcript:-not found}"
} > "$run_dir/meta.txt"
