...
`boxr skill install --harness claude|codex|pi|all` writes each bundled skill into that harness's user-level skill directory, and omitting `--harness` covers every supported harness.
