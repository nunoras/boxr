...
- `summary.jsonl` is append-only and every reader takes the last line for an id, so `boxr outcome` and `boxr outcome --check-reverted` append a new record instead of rewriting the file; never reintroduce a whole-file rewrite, which drops a session that finished concurrently.
- A session's outcome signals stay separate fields rather than one rolled-up status; git evidence is the commits reachable from the exit `HEAD` that the launch `HEAD` did not reach, and `--check-reverted` marks a commit reverted only when no local branch contains it.
