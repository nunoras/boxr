...
- API-equivalent cost is estimated per session from `currency` and `prices` in config (`src/cost.rs`), recorded in the summary as `apiEquivalentCost` plus `currency`, and summed by `boxr stats`; a model missing from the table records null plus a `costError` naming the model, never zero, and stats counts those as `unpricedSessions` through the separate `costUnpriced` flag.
